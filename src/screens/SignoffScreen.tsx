...
import { getAudit, signAudit, signAuditAsAuditor } from '../services/audits';
...
export default function SignoffScreen({ route, navigation }: Props) {
  ...
  const [confirmingAuditor, setConfirmingAuditor] = useState(false);
  const [auditorSignedAt, setAuditorSignedAt] = useState<number | null>(null);
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!a) return;
      setAudit(a);
      setAuditorSignedAt(a.auditorSignedAt);
      setSignedAt(a.signedAt);
      setLoading(false);
    })();
  }, [auditId]);

  async function confirmAsAuditor() {
    setConfirmingAuditor(true);
    try {
      const at = await signAuditAsAuditor(auditId, auditorName || 'Auditor');
      setAuditorSignedAt(at);
    } finally {
      setConfirmingAuditor(false);
    }
  }
        ...
        {!auditorSignedAt ? (
          <View style={{ gap: 10 }}>
            <View style={styles.card}>
              <Text style={styles.cardKicker}>Auditor confirmation</Text>
              <Text style={styles.cardName}>{auditorName || 'Auditor'}</Text>
              <Text style={styles.cardBody}>
                Confirm you observed this assessment and agree the result recorded above: awarded level{' '}
                {audit.awardedLevel} against an expected {audit.expectedLevel}
                {gap < 0 ? ` (${gap})` : ''}.
              </Text>
            </View>
            <Button label="Confirm as auditor" onPress={confirmAsAuditor} loading={confirmingAuditor} fullWidth />
          </View>
        ) : (
          <>
            <View style={styles.confirmedStrip}>
              <Text style={styles.confirmedStripText}>
                ✓ Auditor confirmed {formatSignedTime(new Date(auditorSignedAt))}, {formatFullDate(auditorSignedAt)}
              </Text>
            </View>
              ...
              <View style={{ gap: 20 }}>
                <View style={styles.signedCard}>
                  ...
                  <Text style={styles.signedNote}>
                    Recorded against this audit only. Signed by {audit.candidateName} on this device. Locked —
                    reopening needs a supervisor.
                  </Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  cardKicker: { fontFamily: font.semibold, fontSize: 10.5, letterSpacing: 1.5, color: color.neutral600, textTransform: 'uppercase' },
  ...
  confirmedStrip: { backgroundColor: color.surface, padding: 10 },
  confirmedStripText: { fontFamily: font.semibold, fontSize: 12.5, color: color.text },
  confirmBtn: {
    position: 'relative',
    overflow: 'hidden',
    minHeight: 72,
